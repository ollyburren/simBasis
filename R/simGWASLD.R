...
simCVLDBlock <- function(ld.cmd,snps.DT,scenarios,nsim,prune=TRUE){
  ...
  use <- apply(h,2,var)>0
  h <- h[,use,drop=FALSE]
  ## across all input scenarios get a list of CV's
  all.cv <- lapply(scenarios,'[[','CV') %>% do.call('c',.)
  basis_variants <- c(snps.DT[asb==TRUE,]$id,all.cv) %>% unique
  h <- h[,which(colnames(h) %in% basis_variants)]
  variants <- colnames(h)
